	...
	query := fmt.Sprintf(`
{
	...
	if count > 0 {
		first := pools[0]
		last := pools[count-1]
		fmt.Printf("✅ 获取成功！Pool 数量: %d\n", count)
		fmt.Printf("   第一条: %s (%s/%s) feeTier=%s\n", first.ID, first.Token0.Symbol, first.Token1.Symbol, first.FeeTier)
		fmt.Printf("   最后一条: %s (%s/%s) feeTier=%s\n", last.ID, last.Token0.Symbol, last.Token1.Symbol, last.FeeTier)
	}
